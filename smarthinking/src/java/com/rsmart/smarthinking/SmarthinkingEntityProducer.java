...
/**
 * @author hedrick The goal is to get sites to save and copy. However there's
 *         actually no data involved in this tool. The only configuration is the
 *         URL, which is a tool configuration property. That's handled
 *         separately in site.xml
 * 
 */
public class SmarthinkingEntityProducer implements EntityProducer,
		EntityTransferrer, Serializable {

	protected final Log logger = LogFactory.getLog(getClass());

	private static final String ARCHIVE_VERSION = "2.4"; // in case new features
															// are added in
															// future exports
	...
	public static final String REFERENCE_ROOT = "/smarthinking";
	public static final String LINKTOOL_ID = "rsmart.linktool.smarthinking";
	public static final String SMARTHINKING = "smarthinking";
	...
	public void init() {
		...
		try {
			...
			logger.warn("Error registering Smarthinking Entity Producer", e);
		}

		try {
			ComponentManager.loadComponent(
					"com.rsmart.smarthinking.SmarthinkingEntityProducer", this);
		} catch (Exception e) {
			logger.warn(
					"Error registering Smarthinking Tool Entity Producer with Spring. Suggest restarting Sakai",
					e);
		}
	...
	protected String serviceName() {
		return SmarthinkingEntityProducer.class.getName();
	}
		...
		try {
			...
			if (tools != null && !tools.isEmpty()) {
				for (ToolConfiguration config : tools) {
					...
					if (heights != null) {
						...
						if (heights.endsWith("px"))
							heights = heights
									.substring(0, heights.length() - 2).trim();
						height = heights;
					}
				...
				results.append("archiving " + getLabel() + ": (" + tools.size()
						+ ") smarthinking instances archived successfully.\n");

			} else {
				results.append("archiving " + getLabel() + ": no smarthinking.\n");
			}
			...
			logger.warn("archive: exception archiving service: "
					+ serviceName());
		}
	...
	public String merge(String siteId, Element root, String archivePath,
		...
		logger.debug("trying to merge smarthinking");
			...
			try {
				...
				for (int i = 0; i < length; i++) {
					...
					if (siteNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						if (element.getTagName().equals(SMARTHINKING)) {
							Site site = SiteService.getSite(siteId);

							// in the model, this code was there. It would
							// require
							// one tool to already be present. I'm not sure
							...
							if (true) {
								...
								for (int cn = 0; cn < lengthNodes; cn++) {
									...
									if (node.getNodeType() == Node.ELEMENT_NODE) {
										...
										if (linkElement.getTagName().equals(
												SMARTHINKING)) {
											String toolId = linkElement
													.getAttribute("toolid");
				...
				results.append("merging smarthinking " + siteId + " (" + count
						+ ") items.\n");
	...
	public void transferCopyEntities(String fromContext, String toContext,
			List ids) {
		logger.debug("smarthinking transferCopyEntities");
		try {
			...
			if (fromSitePages != null && !fromSitePages.isEmpty()) {
				...
				while (pageIter.hasNext()) {
					...
					while (toolIter.hasNext()) {
						ToolConfiguration toolConfig = (ToolConfiguration) toolIter
								.next();

						if (toolIds.contains(toolConfig.getToolId())) {
							String contentUrl = toolConfig.getPlacementConfig()
									.getProperty("url");
							String toolTitle = toolConfig.getTitle();
							String pageTitle = currPage.getTitle();
							String height = toolConfig.getPlacementConfig()
									.getProperty("height");
			...
			logger.warn(
					"transferCopyEntities(): exception in handling webcontent data: ",
					any);
		}
